...
from PySide6.QtWidgets import (
    ...
    QPushButton, QFileDialog, QVBoxLayout, QComboBox, QSpinBox, QDateEdit, QMessageBox
)
...
class DialogoAggiuntaSoci(QDialog):
    ...
    def calcola_cf_automaticamente(self):
        if self.nazione.currentText().lower() != "italia":
            QMessageBox.information(self, "Info", "Il calcolo del codice fiscale è disponibile solo per cittadini italiani.")
            return
        nome = self.nome.text()
        ...
        if not all([nome, cognome, data, luogo]):
            QMessageBox.warning(self, "Dati mancanti", "Inserisci tutti i campi necessari per il calcolo.")
            return
        ...
        if cf:
            ...
            QMessageBox.warning(self, "Errore", "Non è stato possibile calcolare il codice fiscale.")

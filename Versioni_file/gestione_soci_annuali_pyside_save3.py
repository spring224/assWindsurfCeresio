...
class FinestraGestioneSoci(QWidget):
    ...
    # Logo
     ...
     # Dati socio
        ...
        c.drawString(text_x + 25 * mm, text_y, socio['id'])

        text_y -= 5 * mm
        c.setFont('Helvetica-Bold', 8)
        c.drawString(text_x, text_y, "Nome Associato:")
        c.setFont('Helvetica', 8)
        c.drawString(text_x + 25 * mm, text_y, f"{socio['nome']} {socio['cognome']}")

        text_y -= 5 * mm
        c.setFont('Helvetica-Bold', 8)
        c.drawString(text_x, text_y, "Anno Validità:")
        c.setFont('Helvetica', 8)
        c.drawString(text_x + 25 * mm, text_y, str(socio['anno']))

    # Foto placeholder
        photo_x = CARD_WIDTH - 30 * mm - 5 * mm
        photo_y = CARD_HEIGHT - 35 * mm - 5 * mm
        photo_width = 30 * mm
        photo_height = 35 * mm
